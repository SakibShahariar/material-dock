...
export class GlobalSignalsHandler {
    ...
    add(...args) {
        const entries = args.length === 1 ? args[0] : [args];
        for (const entry of entries) {
            const [obj, signalName, callback] = entry;
...
export function formatRelativeTime(mtimeSec) {
    const diff = Math.floor(GLib.get_real_time() / 1000000) - mtimeSec;
...
export function getAppWindows(app) {
    const tracker = Main.windowTracker;
