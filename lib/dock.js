...
import {GlobalSignalsHandler, log} from './utils.js';

const REVEAL_STRIP = 8;
const ANIM_TIME = 180;

export const MaterialDock = GObject.registerClass(
class MaterialDock extends St.Bin {
    _init(settings, monitorIndex, onOpenPreferences) {
        ...
        this._dockPosition = settings.get_string('dock-position');
    ...
    _resetPosition() {
        ...
        const kids = this._contentBox.get_children().map(
            c => `${c.constructor.name}:${c.get_name() ?? c.style_class ?? ''}:W${c.get_preferred_width(-1)[1]}:H${c.get_preferred_height(-1)[1]}`);
        const dashKids = this._dash._box?.get_children().length ?? -1;
        log(`POS dock prefW=${natWidth} prefH=${natHeight} iconSize=${this.iconSize} contentKids=[${kids}] dashItemCount=${dashKids}`);

        const alignment = this.settings.get_string('dock-alignment');
        const margin = this.settings.get_int('dock-margin');

        let x, y;
        if (this._dockPosition === 'top') {
    ...
    _updateAutoFit() {
        ...
        try {
            const configured = this.settings.get_int('icon-size');
            const autoFit = this.settings.get_boolean('auto-fit-icons');
            const current = this._dash.iconSize || configured;

            if (!autoFit) {
                if (current !== configured)
                    this._dash.setIconSize(configured);
                return;
            }
            ...
            const margin = this.settings.get_int('dock-margin');
            // Leave room for card padding, stacks, minimized tiles and a little breathing room
            const available = Math.max(120, monitor.width - 2 * margin - 48);

            const [, natWidth] = this.get_preferred_width(-1);
            if (natWidth <= 0)
                return;

            if (natWidth <= available) {
                // Room to grow back toward the user-configured size
                if (current < configured)
                    this._dash.setIconSize(configured);
                return;
            }

            // Shrink proportionally, snap to multiples of 4, never below 24
            let newSize = Math.floor(current * available / natWidth);
            newSize = Math.max(24, Math.min(configured, newSize));
            newSize = Math.floor(newSize / 4) * 4;
            if (newSize < 24)
                newSize = 24;

            if (newSize !== current)
                this._dash.setIconSize(newSize);
        } finally {
            this._fitting = false;
        }
    }
    ...
    _onSettingChanged(key) {
        switch (key) {
        case 'dock-position':
            this._dockPosition = this.settings.get_string('dock-position');
            this._resetPosition();
            this._updateStaticBox();
            break;
        case 'dock-alignment':
    ...
    onAppItemCreated(item, app) {
        ...
        const ids = [
            ...
            item.connect('notify::mapped', stateHandler),
        ];
        ...
        if (iconWidget) {
            iconWidget.connect('notify::hover', () => {
                log(`HOVER mag item=${iconWidget.app?.get_id()?.slice(0,20)} hover=${iconWidget.hover}`);
            ...
            // Preview on hover; closed by the poll once the pointer leaves the
            // dock/panel corridor (see PreviewPopup._startPoll).
            iconWidget.connect('notify::hover', () => {
                const showing = this.settings.get_boolean('show-previews');
                log(`HOVER preview app=${app.get_id()?.slice(0,20)} hover=${iconWidget.hover} showing=${showing}`);
                if (iconWidget.hover && showing)
    ...
    _updateIndicator(item, app) {
        ...
        const urgent = windows.some(w => w.demands_attention);
        ...
        log(`IND item=${app.get_id()?.slice(0,20)} running=${running} focused=${focused} style=${style} itemStage=${ix},${iy} itemwh=${item.width}x${item.height} indwh=${indicator.width}x${indicator.height}`);
    }
    ...
    onAppIconClick(icon, button) {
        ...
        log(`CLICK app=${app.get_id()?.slice(0,20)} button=${button} action=${action} state=${app.state}`);
    ...
    _activateFallback(app) {
        ...
        let target = windows.find(w =>
            w.workspace === activeWs && !w.minimized &&
            w !== global.display.focus_window);
        if (!target)
            target = windows.find(w => w.workspace === activeWs);
        if (!target)
            target = windows[0];
    ...
    _minimizeAllWindows(app) {
        ...
        const windows = app.get_windows().filter(w =>
            !w.skip_taskbar && w.workspace === activeWs &&
    ...
    _onPointerMove(x, y) {
        ...
        const inStrip = this._dockPosition === 'bottom'
    ...
    _setVisible(visible) {
        ...
        const margin = this.settings.get_int('dock-margin');
        const offset = (height + margin) * (this._dockPosition === 'top' ? -1 : 1);
