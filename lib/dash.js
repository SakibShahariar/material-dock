...
class MaterialDashItemContainer extends St.Widget {
    ...
    setChild(actor) {
        ...
        this.child.x_expand = false;
        this.child.x_align = Clutter.ActorAlign.CENTER;
        this.child.y_align = Clutter.ActorAlign.START;
...
class MaterialDash extends Dash {
    _init(dockOwner, settings) {
        ...
        this._dashContainer.y_align = Clutter.ActorAlign.START;

        // App icons are top-aligned and carry a running indicator below the
        // texture. Match that baseline for the app-grid button so the icon
        // surfaces line up, and keep a clear gap after the last app icon.
        this._showAppsIcon.y_align = Clutter.ActorAlign.START;
        this._showAppsIcon.x_align = Clutter.ActorAlign.CENTER;
        this._dashContainer.spacing = Math.max(10, Math.round(this.iconSize / 3));
    }
